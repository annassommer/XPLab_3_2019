...
author: "Anna Sommer & Daniel Pietschke"
output:
  html_document
---
...
summarize(tooth_growth_tibble, mean = mean(len))
...
ggplot(ToothGrowth, aes(dose,len)) + geom_point(aes(color = supp), size = 3) + xlab("Dose") + ylab("Tooth Length") + scale_color_discrete(name = "Supplement", labels = c("Orange Juice", "Ascorbic Acid"))
...
There seems to be a correlation between the dose of the supplement and the tooth growth, however at a small dose orange juice seems to be more effective than asorbic acid. Though this makes the impression of leveling out, as the dose grows.
